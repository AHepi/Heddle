...
A package root holds `skills/*.md`, `flows/*.yaml`, `tools.yaml`, and
`grants.yaml`. `run` takes a script of canned model replies; for real runs,
wire a model callable into `heddle.Interpreter(pkg, model, human, filters)`,
where `model(prompt, tools, step_ref)` returns
`{"outcome": ..., "tool_calls": [...]}`.

Depends on PyYAML. Tests: `pytest tests_heddle` (16 tests).
