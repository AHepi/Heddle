...
Style*. Shipped as a distributable package, not into the root package
— the root is this repo's own discipline, and a prose-polish workflow
is exactly the packages/ tree's purpose. packages/style/ is a complete
self-contained root: skills/critique.md + skills/rewrite.md (one step
per file, name = step = filename), flows/style.yaml (critique →
rewrite only when critique reports needs_work), tools.yaml
(read_draft/write_draft rooted at drafts/), grants.yaml (role
stylist: read at both steps, write_only write at rewrite only), a
README, and a deliberately bad seed draft so a first run has something
to chew.

The book, distilled into the two steps: critique works the checklist
biggest-gears-first — classic style (metadiscourse, signposting,
professional narcissism), the curse of knowledge (unglossed chunks),
zombie nouns and hedges, the tree (working memory, given-before-new,
light-before-heavy), arcs of coherence, and the usage rulings (break
the folklore rules freely; hold the line on disinterested, enormity,
fortuitous, refute and kin) — with every finding required to quote the
draft's own words. rewrite re-sees rather than synonym-swaps, preserves
meaning exactly (gaps marked, not invented), keeps deliberate passives,
and writes a NEW note: the before/after pair is the evidence the pass
happened. The stylist role cannot write at critique — the grant, not
the prose, enforces the diagnose/fix split.

Instruments: gate 38 passed, 0 failed; wheel 5 passed; packages wheel
13 passed; `heddle check` (installed) clean on style plus triage,
publish, postmortem standalone; `heddle show` confirms critique sees
exactly read_draft and rewrite exactly read_draft+write_draft;
scripted live runs rc 0 on both paths — the needs_work path wrote
example-draft-revised.md beside an untouched original (critique's
scripted write attempt was not visible and was dropped by CannedModel,
zero denials by design), the sound path ran critique alone. Trap
census: TRAP-inventory-pin-lags-surface applies — the PACKAGES pin
moved in this commit; TRAP-flow-vacuous-keys guarded by the wheel's
non-vacuous parse test; TRAP-self-host-runtime-dir — all checks run
from the installed lib; no other matches. Tripwire: heddle/ and
HEDDLE_0_1.md untouched; map/ change is this ledger entry only. MISS,
recorded per the house rules: the push tool call was assembled by hand
and its file array carried only the seven packages/ paths, so the pin
(test_packages.py), the root README, and this ledger entry landed one
commit later — the branch's gate would have been red between the two
pushes (PACKAGES pin vs. disk), caught here by re-reading the push
response rather than by CI. Same class as
TRAP-inventory-pin-lags-surface (surface and pin in different commits),
different mechanism: assembly, not forgetfulness. A second miss
compounded it: the fix push went out with a placeholder payload that
clobbered README.md, restored in the same fix-forward commit. Both
misses trace to hand-assembled push payloads; the prevention is to
build the file array from disk mechanically (as the local staging did)
and re-read the push response before reporting.

Residue: never run against a live model — the scripted runs prove
control flow and per-step visibility only, not that a model applies
Pinker well (the checklist is prose, not machinery; the quote-the-line
rule is the backstop). Skill names critique/rewrite may collide on
adoption — the loader's duplicate error catches it and packages/README
documents the rename path. run artifacts (script.yaml, the revised
seed note) were exercised and removed; they are not shipped.
...
live model; summary filters still unshipped; the vacuous-keys hole in the
validator itself is guarded by the wheel but not yet fixed in
